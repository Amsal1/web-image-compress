...
describe('validateFiles', () => {
  it('accepts supported MIME types', () => {
    ...
    const result = validateFiles(files, 400_000);
    expect(result.valid).toHaveLength(5);
    expect(result.rejected).toHaveLength(0);
  });

  it('rejects unsupported MIME types with descriptive message', () => {
  ...
  it('returns empty arrays for empty input', () => {
    const result = validateFiles([], 400_000);
    expect(result.valid).toHaveLength(0);
    expect(result.rejected).toHaveLength(0);
  });

  it('preserves the correct format on valid files', () => {
